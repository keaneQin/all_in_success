...
export const recommendationApi = {
  getRecommendations: async (): Promise<RecommendationResponse> => {
    const response = await apiClient.get('/api/v1/analysis/recommendations');
    return toCamelCase(response.data);
  },
  getAfternoonRecommendations: async (): Promise<RecommendationResponse> => {
    const response = await apiClient.get('/api/v1/analysis/recommendations/afternoon');
    return toCamelCase(response.data);
  },
};
