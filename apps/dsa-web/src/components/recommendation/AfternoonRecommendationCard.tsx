...
const DetailModal: React.FC<{ stock: RecommendationStock | null; onClose: () => void }> = ({ stock, onClose }) => {
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose} style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
      <div className="dashboard-card w-full max-w-md p-5 animate-fade-in" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button type="button" onClick={onClose} className="home-surface-button flex items-center rounded-lg p-1.5 text-secondary-text"><X className="h-4 w-4" /></button>
        </div>
        <div className="mb-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className={`text-lg font-semibold font-mono ${stock.changePct > 0 ? 'text-success' : stock.changePct < 0 ? 'text-danger' : 'text-muted-text'}`}>{stock.changePct > 0 ? '+' : ''}{stock.changePct.toFixed(2)}%</p>
          </div>
        </div>
        <div>
          <span className="text-[11px] text-muted-text mb-1 block">筛选理由</span>
          <div className="home-subpanel px-3 py-2.5">
            <p className="text-xs leading-relaxed text-secondary-text whitespace-pre-wrap">{stock.reason}</p>
          </div>
        </div>
        <p className="mt-4 text-[9px] text-muted-text text-center opacity-60">基于 AI 策略筛选 · 仅供参考，投资需谨慎</p>
      </div>
    </div>
  );
};

const AFTERNOON_KEY = 'ais_aft_stocks';

export const AfternoonRecommendationCard: React.FC = () => {
  const [stocks, setStocks] = useState<RecommendationStock[]>(() => {
    try {
      const saved = sessionStorage.getItem(AFTERNOON_KEY);
  ...
  const fetch = async () => {
    ...
    setError(null);
    try {
      const res = await recommendationApi.getAfternoonRecommendations();
      const data = res.stocks;
      setStocks(data);
      try { sessionStorage.setItem(AFTERNOON_KEY, JSON.stringify(data)); } catch {}
    } catch (err: unknown) {
    ...
    <>
      <div className="dashboard-card p-3 animate-fade-in">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-foreground">🌆 尾盘推荐</span>
            <StatusDot tone={loading ? 'warning' : 'success'} />
          </div>
          <button type="button" onClick={fetch} disabled={loading} className="home-surface-button flex items-center gap-1 rounded-lg px-2 py-1 text-[11px] text-secondary-text">
        ...
        {error ? (
          ...
          <div className="flex items-center justify-center py-8"><div className="home-spinner h-5 w-5 border-2" /></div>
        ) : stocks.length === 0 ? (
        ...
        <p className="mt-3 text-[9px] text-muted-text text-center opacity-60">基于 AI 策略筛选 · 仅供参考，投资需谨慎</p>
      </div>
