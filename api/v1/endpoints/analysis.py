...
# ============================================================
# GET /recommendations - 新手股票推荐
# ============================================================

def _load_skills(filename: str = 'skills.md') -> list:
    rules = []
    skills_path = f'/opt/daily_stock_analysis/{filename}'
...
def _fetch_stock_price(code: str) -> Optional[Dict]:
    try:
        prefix = 'sh' if code.startswith('6') or code.startswith('9') else 'sz'
        r = requests.get(f"http://qt.gtimg.cn/q={prefix}{code}", timeout=10, headers={'User-Agent': 'Mozilla/5.0'})
        if r.status_code == 200 and '~' in r.text:
            p = r.text.split('~')
            if len(p) >= 40:
                return {
                    'code': p[2], 'name': p[1],
                    'current_price': float(p[3]) if p[3] else 0,
                    'change_pct': float(p[32]) if p[32] else 0,
                    'change': float(p[31]) if p[31] else 0,
                }
    except:
        pass
    return None
...
@router.get(
    "/recommendations",
    summary="获取新手股票推荐",
    description="基于 skills.md 筛选规则和 AI 分析，推荐 3 只适合新手的 A 股股票",
)
def get_recommendations():
    config = get_config_dep()
    load_dotenv('/opt/daily_stock_analysis/.env')
    llm_api_key = os.getenv('DEEPSEEK_API_KEY', '') or os.getenv('LLM_API_KEY', '')
    llm_model = 'deepseek-v4-flash'

    skills_rules = _load_skills()
    recommendations = []

    MAX_PRICE = 50.0

    if llm_api_key and skills_rules:
        try:
            skill_text = '\n'.join(skills_rules)
            client = OpenAI(api_key=llm_api_key, base_url="https://api.deepseek.com")
            prompt = f"""你是一位专业股票分析师。请根据以下筛选规则，推荐符合条件的前5只A股股票：

筛选规则：
{skill_text}

请以JSON格式返回推荐结果，每只股票需包含：
{{"stocks":[{{"name":"股票名称","code":"6位代码","reason":"该股票符合哪些筛选规则的具体分析，包含数据支撑"}}]}}
推荐5只，按推荐优先级排序。必须是A股真实存在的股票。"""
            r = client.chat.completions.create(
                model=llm_model,
                messages=[{"role":"system","content":"你是一名专业的A股股票分析师，擅长根据条件筛选股票。请严格按照JSON格式输出，不要输出其他内容。"}, {"role":"user","content":prompt}],
            ...
            if m:
                ...
                for item in data.get('stocks', []):
                    ...
                    if not code or not name:
                        continue
                    price_data = _fetch_stock_price(code)
                    price = price_data['current_price'] if price_data else 0
                    if price > MAX_PRICE:
                        continue
                    rec = {
                        'name': name, 'code': code,
            ...
            logger.warning(f"AI推荐失败: {e}")

    return {"stocks": recommendations[:3]}


@router.get(
    "/recommendations/afternoon",
    summary="获取尾盘推荐",
    description="基于 skills-old.md 筛选规则和 AI 分析，推荐 3 只尾盘可关注的 A 股股票",
)
def get_afternoon_recommendations():
    ...
    skills_rules = _load_skills('skills-old.md')
    recommendations = []
    ...
    if llm_api_key and skills_rules:
        try:
            skill_text = '\n'.join(skills_rules)
            client = OpenAI(api_key=llm_api_key, base_url="https://api.deepseek.com")
            prompt = f"""你是一位专业股票分析师。请根据以下尾盘筛选规则，推荐符合条件的前3只A股股票：
...
请以JSON格式返回推荐结果，每只股票需包含：
{{"stocks":[{{"name":"股票名称","code":"6位代码","reason":"该股票符合哪些筛选规则的具体分析，包含数据支撑"}}]}}
推荐3只，按推荐优先级排序。必须是A股真实存在的股票。"""
            r = client.chat.completions.create(
                model=llm_model,
                messages=[{"role":"system","content":"你是一名专业的A股股票分析师，擅长尾盘选股。请严格按照JSON格式输出，不要输出其他内容。"}, {"role":"user","content":prompt}],
                temperature=0.1, max_tokens=2000,
                timeout=60
            )
            text = r.choices[0].message.content
